...
def get_sum_of_sample_mean(
    ...
    if sample_weights is None:
        sample_weights = [1.0] * len(response_lengths)
    if cp_size == 1:

        def sum_of_sample_mean(x: torch.Tensor) -> torch.Tensor:
            return sum(
                [
                    ((x_i * loss_mask_i).sum() / torch.clamp_min(loss_mask_i.sum(), 1)) * sample_weight_i
                    for x_i, loss_mask_i, sample_weight_i in zip(
                        x.split(response_lengths, dim=0), loss_masks, sample_weights, strict=False
                    )
                ]
            )
        ...
        def sum_of_token(x: torch.Tensor) -> torch.Tensor:
            return sum(
                [
                    (x_i * loss_mask_i).sum() * sample_weight_i
                    for x_i, loss_mask_i, sample_weight_i in zip(
                        x.split(response_lengths, dim=0), loss_masks, sample_weights, strict=False
                    )
                ]
        ...
        def sum_of_sample_mean(x: torch.Tensor) -> torch.Tensor:
            return sum(
                [
                    ((x_i * chunked_loss_mask).sum() / torch.clamp_min(loss_mask.sum(), 1)) * sample_weight_i
                    for x_i, chunked_loss_mask, loss_mask, sample_weight_i in zip(
                        x.split(cp_chunk_lengths, dim=0), chunked_loss_masks, loss_masks, sample_weights, strict=False
                    )
                ]
            )
        ...
        def sum_of_token(x: torch.Tensor) -> torch.Tensor:
            return sum(
                [
                    (x_i * chunked_loss_mask).sum() * sample_weight_i
                    for x_i, chunked_loss_mask, sample_weight_i in zip(
                        x.split(cp_chunk_lengths, dim=0), chunked_loss_masks, sample_weights, strict=False
                    )
                ]
            )
