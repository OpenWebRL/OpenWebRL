...
def train_one_step(
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        if args.calculate_per_token_loss:
            loss_reduced["raw_loss_tokens"] = num_samples_or_tokens
        else:
...
def train(
    ...
    Args:
        ...
        ppo_epoch_id (int): PPO epoch index over the same rollout data.
    """
    args = get_args()
    ...
    for step_id in range(num_steps_per_rollout):
        if (
            ...
            logger.info(
                "train_one_step start rollout_id=%s ppo_epoch=%s step_id=%s num_microbatches=%s",
                rollout_id,
                ppo_epoch_id,
                step_id,
        ...
        if (
            ...
            logger.info(
                "train_one_step end rollout_id=%s ppo_epoch=%s step_id=%s grad_norm=%s",
                rollout_id,
                ppo_epoch_id,
                step_id,
                grad_norm,
        ...
        if (
            ...
            accumulated_step_id = (
                rollout_id * args.ppo_epochs * num_steps_per_rollout + ppo_epoch_id * num_steps_per_rollout + step_id
            )
            ...
            log_dict["train/ppo_epoch"] = ppo_epoch_id
            if args.enable_mtp_training:
            ...
            for key in [
                ...
                if value is not None:
                    progress_fields.append(f"{key.split('/')[-1]}={value:.6g}")
            if progress_fields:
                line = (
                    f"[TrainMetrics] rollout={rollout_id + 1} "
                    f"step={accumulated_step_id} role={role} ppo_epoch={ppo_epoch_id} "
