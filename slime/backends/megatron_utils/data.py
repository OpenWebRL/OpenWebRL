...
def get_batch(
    ...
    if "effective_global_batch_size" in data_iterator.rollout_data:
        batch["effective_global_batch_size"] = data_iterator.rollout_data["effective_global_batch_size"]
...
def _get_image_pad_token_id(args: Namespace) -> int | None:
    tokenizer_model = getattr(args, "tokenizer_model", None) or getattr(args, "hf_checkpoint", None)
    ...
    try:
        ...
        if len(token_ids) == 1:
            ...
            logger.warning(
                "Megatron image-token debug could not resolve a unique <|image_pad|> token id from %s: %s",
                tokenizer_model,
                token_ids,
            )
    except Exception as exc:
        logger.warning(
            "Megatron image-token debug failed to load tokenizer from %s: %s",
            tokenizer_model,
            exc,
        )
...
def _extract_image_debug_stats(
    ...
    if mm_input_dict is not None and "image_grid_thw" in mm_input_dict and mm_input_dict["image_grid_thw"] is not None:
    ...
    return {
        ...
        "image_pad_match": int(
            image_pad_token_id is not None and expected_visual_tokens == actual_image_pad_tokens
        ),
        "mm_missing_but_has_image_tokens": int(has_mm == 0 and actual_image_pad_tokens > 0),
        "mm_present_but_no_image_tokens": int(has_mm == 1 and actual_image_pad_tokens == 0),
...
def log_megatron_image_token_debug(
    rollout_id: int,
    args: Namespace,
    ...
    if mm_inputs is None:
        mm_inputs = [None] * len(token_tensors)
    else:
        assert len(mm_inputs) == len(token_tensors), (
            f"multimodal_train_inputs length {len(mm_inputs)} != tokens length {len(token_tensors)}"
        )
    ...
    num_samples = len(stats_per_sample)
    mm_ratio = sum(sample["has_mm"] for sample in stats_per_sample) / max(num_samples, 1)
    image_token_ratio = sum(1 for sample in stats_per_sample if sample["actual_image_pad_tokens"] > 0) / max(num_samples, 1)
    broken_ratio = sum(sample["mm_missing_but_has_image_tokens"] for sample in stats_per_sample) / max(num_samples, 1)
    mismatch_ratio = sum(sample["mm_present_but_count_mismatch"] for sample in stats_per_sample) / max(num_samples, 1)

    first_problem = next(
        (
            sample_indices[i] if sample_indices is not None else i
            for i, sample in enumerate(stats_per_sample)
            if sample["mm_missing_but_has_image_tokens"] or sample["mm_present_but_count_mismatch"]
        ),
        None,
    )
    ...
    log_fn(
        "[MegatronImageTokenDebug] rollout=%s image_pad_token_id=%s samples=%s mm_ratio=%.3f image_token_ratio=%.3f broken_ratio=%.3f mismatch_ratio=%.3f first_problem_sample=%s",
        rollout_id,
        image_pad_token_id,
        num_samples,
        mm_ratio,
...
def get_data_iterator(
    ...
    effective_global_batch_size = rollout_data.get("effective_global_batch_size")
    dynamic_global_batch_size = rollout_data.get("dynamic_global_batch_size")
    global_batch_size = effective_global_batch_size or dynamic_global_batch_size or args.global_batch_size
    num_local_gbs = global_batch_size // dp_size
    num_steps_per_rollout = num_local_samples // num_local_gbs

    if effective_global_batch_size is not None:
        logger.info(
            f"Using epoch effective_global_batch_size={global_batch_size} "
            f"(static_global_batch_size={args.global_batch_size}, "
            f"rollout_dynamic_global_batch_size={dynamic_global_batch_size}), "
            f"num_local_samples={num_local_samples}, num_steps_per_rollout={num_steps_per_rollout}"
        )
    elif global_batch_size != args.global_batch_size:
        logger.info(
            f"Using rollout dynamic_global_batch_size={global_batch_size} "
            f"(static_global_batch_size={args.global_batch_size}), "
            f"num_local_samples={num_local_samples}, num_steps_per_rollout={num_steps_per_rollout}"
        )
    ...
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        ...
        for key, val in rollout_data.items():
            ...
            log_dict |= compute_pass_rate(
                ...
                group_sizes=rollout_data.get("raw_reward_group_sizes"),
            )
