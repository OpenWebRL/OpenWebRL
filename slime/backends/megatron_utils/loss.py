...
def policy_loss_function(
    ...
    abs_advantage = sum_of_sample_mean(advantages.abs())
    ...
    reported_loss = {
        ...
        "abs_advantage": abs_advantage.clone().detach(),
...
def sft_loss_function(
    ...
    loss_token_counts = [loss_mask.sum() for loss_mask in batch["loss_masks"]]
    loss_tokens = torch.stack(loss_token_counts).sum()
    response_tokens = torch.tensor(sum(response_lengths), device=logits.device, dtype=torch.float32)
    total_tokens = torch.tensor(sum(total_lengths), device=logits.device, dtype=torch.float32)
    max_response_tokens = torch.tensor(max(response_lengths), device=logits.device, dtype=torch.float32)
    max_total_tokens = torch.tensor(max(total_lengths), device=logits.device, dtype=torch.float32)
    max_loss_tokens = torch.stack(loss_token_counts).max().to(dtype=torch.float32)
    mean_loss_tokens = loss_tokens / max(len(loss_token_counts), 1)
    ...
    if logger.isEnabledFor(logging.INFO):
        logger.info(
            "sft_loss stats: samples=%s total_tokens=%s response_tokens=%s loss_tokens=%s "
            "max_total_tokens=%s max_response_tokens=%s max_loss_tokens=%s",
            len(response_lengths),
            int(total_tokens.item()),
            int(response_tokens.item()),
            int(loss_tokens.item()),
            int(max_total_tokens.item()),
            int(max_response_tokens.item()),
            int(max_loss_tokens.item()),
        )
    ...
    return (
        ...
        {
            "loss": loss.clone().detach(),
            "sft_loss_tokens": loss_tokens.clone().detach(),
            "sft_response_tokens": response_tokens.clone().detach(),
            "sft_total_tokens": total_tokens.clone().detach(),
            "sft_max_loss_tokens": max_loss_tokens.clone().detach(),
            "sft_max_response_tokens": max_response_tokens.clone().detach(),
            "sft_max_total_tokens": max_total_tokens.clone().detach(),
            "sft_mean_loss_tokens": mean_loss_tokens.clone().detach(),
        },
    )
...
def loss_function(
    ...
    global_batch_size = batch.get(
        "effective_global_batch_size",
        batch.get("dynamic_global_batch_size", args.global_batch_size),
    )
