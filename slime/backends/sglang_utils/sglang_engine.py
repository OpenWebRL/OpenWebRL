...
def launch_server_process(server_args: ServerArgs, worker_type: str = "regular") -> multiprocessing.Process:
    # `encoder_only` exists in some SGLang builds but is absent in others.
    # Guard the access so regular rollout servers still launch on newer builds.
    encoder_only = getattr(server_args, "encoder_only", False)
    if worker_type == "encoder" and not encoder_only:
        raise RuntimeError(
            "Installed SGLang ServerArgs does not expose `encoder_only`, "
            "so encoder worker mode is unavailable with this SGLang build."
        )
...
class SGLangEngine(RayActor):
    ...
    def flush_cache(self):
        ...
        for attempt in range(60):
            try:
                response = requests.get(f"http://{self.server_host}:{self.server_port}/flush_cache", timeout=5)
                last_status_code = response.status_code
                last_response_text = response.text
                if response.status_code == 200:
                    break
                logger.info(
                    "flush_cache attempt %s/60 for %s:%s returned status=%s body=%s",
                    attempt + 1,
                    self.server_host,
                    self.server_port,
            ...
            detail = f"last_status={last_status_code}"
            if last_response_text:
                detail += f", last_response={last_response_text[:200]!r}"
            if last_error is not None:
                detail += f", last_error={last_error!r}"
            raise TimeoutError(f"Timeout while flushing cache for {self.server_host}:{self.server_port}. {detail}")
    ...
    def release_memory_occupation(self):
        # Recent SGLang requires release_memory_occupation to be called only
        # when the scheduler is fully idle. Poll /flush_cache first because it
        # returns non-200 while requests are still draining, which gives us a
        # lightweight server-side idle barrier before offload.
        self.flush_cache()
