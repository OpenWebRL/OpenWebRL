...
def run_router(args):
    ...
    # Start the server
    log_level = getattr(args, "slime_router_log_level", None) or os.environ.get("SLIME_ROUTER_LOG_LEVEL", "warning")
    access_log = getattr(args, "slime_router_access_log", None)
    if access_log is None:
        access_log = os.environ.get("SLIME_ROUTER_ACCESS_LOG", "0").strip().lower() in {"1", "true", "yes", "on"}
