...
class RolloutServer:
    ...
    def _mark_engine_dead(self, group: ServerGroup, node0_engine_idx: int, reason: str) -> None:
        """Mark one logical engine dead across all its per-node actors.

        This mirrors the health monitor behavior of setting dead rollout actors
        to ``None`` so later recovery code can recreate them.
        """
        start = node0_engine_idx * group.nodes_per_engine
        end = start + group.nodes_per_engine
        logger.warning(
            "Mark rollout engine dead (worker_type=%s, engine_idx=%s, slots=%s:%s) reason=%s",
            group.worker_type,
            node0_engine_idx,
            start,
            end,
        ...
        for i in range(start, end):
            ...
            if engine is not None:
                try:
                    ...
                    logger.warning("Failed to ray.kill rollout engine actor at index %s: %s", i, exc)
    ...
    def offload(self):
        ...
        return self._run_engine_requests_tolerant(
            op_name="offload",
            request_builder=lambda engine, group: engine.release_memory_occupation.remote(),
        )

    def _run_engine_requests_tolerant(self, op_name: str, request_builder) -> list[tuple[str, int, str]]:
        """Run per-engine RPCs, marking failed engines dead instead of failing fast."""
        handle_meta = {}
        pending = []
        total = 0
        for group in self.server_groups:
            if not group.needs_offload:
                continue
            for node0_engine_idx, engine in enumerate(group.engines):
                if engine is None:
                    continue
                handle = request_builder(engine, group)
                pending.append(handle)
                handle_meta[handle] = (group, node0_engine_idx)
                total += 1
        ...
        while pending:
            ...
            handle = done[0]
            group, node0_engine_idx = handle_meta[handle]
            try:
                ray.get(handle)
            except Exception as exc:
                failures.append((group.worker_type, node0_engine_idx, repr(exc)))
                self._mark_engine_dead(
                    group,
                    node0_engine_idx,
                    reason=f"{op_name} failed: {exc!r}",
                )

        if failures:
            logger.warning("Rollout %s skipped %s dead engine(s): %s", op_name, len(failures), failures)
        elif total > 0:
            logger.info("Rollout %s succeeded for %s/%s engines", op_name, total, total)
        return failures

    def onload(self, tags: list[str] | None = None):
        """Resume memory occupation across all groups (concurrent)."""
        return self._run_engine_requests_tolerant(
            op_name=f"onload(tags={tags})",
            request_builder=lambda engine, group: engine.resume_memory_occupation.remote(tags=tags),
        )
    ...
    def onload_weights(self):
        ...
        """
        return self._run_engine_requests_tolerant(
            op_name="onload_weights",
            request_builder=lambda engine, group: (
                engine.resume_memory_occupation.remote(tags=[GPU_MEMORY_TYPE_WEIGHTS])
                if self.update_weights or not group.model_path
                else engine.update_weights_from_disk.remote(group.model_path)
            ),
        )
    ...
    def onload_kv(self):
        ...
        return self._run_engine_requests_tolerant(
            op_name="onload_kv",
            request_builder=lambda engine, group: engine.resume_memory_occupation.remote(
...
class RolloutManager:
    ...
    def _get_rollout_data(self, rollout_id):
        if self.args.load_debug_rollout_data:
            ...
            def _summarize_group_sizes(samples: list[Sample]) -> dict[str, int | float]:
                if not samples:
                    return {"num_groups": 0, "min": 0, "max": 0, "mean": 0.0}
                grouped = group_by(samples, lambda s: s.group_index)
            ...
            if not self.args.disable_rollout_trim_samples and not self.args.debug_rollout_only:
                ...
                if len(data) % global_batch_size != 0:
                    ...
                    if getattr(self.args, "ppo_epochs", 1) > 1:
                        logger.info(
                            "keeping %d extra rollout samples for rollout_id %s because ppo_epochs=%s; "
                            "training-side epoch shuffle/repartition will decide which samples are skipped each epoch",
                            len(data) - trim_len,
                            rollout_id,
                            self.args.ppo_epochs,
                        )
                    else:
                        origin_data_length = len(data)
                        shuffle_seed = int(getattr(self.args, "rollout_seed", 1) or 1) + int(rollout_id)
                        shuffler = random.Random(shuffle_seed)
                        shuffler.shuffle(data)
                        logger.info(
                            "shuffled %d rollout samples before trim using seed=%d",
                            origin_data_length,
                            shuffle_seed,
                        )
                        before_counts = {gid: len(group) for gid, group in group_by(data, lambda s: s.group_index).items()}
                        data = data[:trim_len]
                        after_counts = {gid: len(group) for gid, group in group_by(data, lambda s: s.group_index).items()}
                        trimmed_groups = [
                            f"{gid}:{after_counts.get(gid, 0)}/{before_count}"
                            for gid, before_count in before_counts.items()
                            if after_counts.get(gid, 0) != before_count
                        ]
                        logger.info(f"trim number of samples from {origin_data_length} to {trim_len}")
                        if trimmed_groups:
                            logger.info(
                                "trimmed group coverage changed for rollout_id %s: %s",
                                rollout_id,
                                ", ".join(trimmed_groups[:16]),
                            )
                        logger.info(
                            "group size summary after trim for rollout_id %s: %s",
                            rollout_id,
                            _summarize_group_sizes(data),
                        )
    ...
    def _post_process_rewards(self, samples: list[Sample] | list[list[Sample]]):
        ...
        def _is_turn_level_sample(sample: Sample) -> bool:
            metadata = sample.metadata or {}
            return "turn_index" in metadata

        def _trajectory_id(sample: Sample) -> Any:
            metadata = sample.metadata or {}
            return metadata.get("trajectory_id", sample.index)
        ...
        if (
            ...
            if group_sizes:
                logger.info(
                    "turn-level reward normalization over %s groups: min_turn_samples=%s max_turn_samples=%s mean_turn_samples=%.2f",
                    len(group_sizes),
                    min(group_sizes),
                    max(group_sizes),
            ...
            is_turn_level = bool(samples) and any(_is_turn_level_sample(sample) for sample in samples)
            if is_turn_level:
                trajectory_group_sizes: list[int] = []
                for group_index, group_samples in grouped_samples.items():
                    trajectory_groups = group_by(group_samples, _trajectory_id)
                    trajectory_group_sizes.append(len(trajectory_groups))

                    trajectory_ids = list(trajectory_groups.keys())
                    trajectory_rewards = []
                    for trajectory_id in trajectory_ids:
                        trajectory_samples = trajectory_groups[trajectory_id]
                        sample_rewards = [sample.get_reward_value(self.args) for sample in trajectory_samples]
                        base_reward = sample_rewards[0]
                        if any(reward != base_reward for reward in sample_rewards[1:]):
                            logger.warning(
                                "Turn-level trajectory reward mismatch detected for group_index=%s trajectory_id=%s; using first reward=%s",
                                group_index,
                                trajectory_id,
                                base_reward,
                            )
                        trajectory_rewards.append(base_reward)

                    rewards_tensor = torch.tensor(trajectory_rewards, dtype=torch.float)
                    rewards_tensor = rewards_tensor - rewards_tensor.mean()
                    if self.args.advantage_estimator in ["grpo", "gspo"] and self.args.grpo_std_normalization:
                        if rewards_tensor.numel() <= 1:
                            logger.warning(
                                "Skipping std normalization for singleton trajectory group group_index=%s num_trajectories=%s",
                                group_index,
                                rewards_tensor.numel(),
                            )
                        else:
                            rewards_tensor = rewards_tensor / (rewards_tensor.std() + 1e-6)

                    for trajectory_id, normalized_reward in zip(trajectory_ids, rewards_tensor.tolist(), strict=True):
                        for sample in trajectory_groups[trajectory_id]:
                            normalized_rewards_by_index[id(sample)] = normalized_reward

                if trajectory_group_sizes:
                    summary_line = (
                        "turn-level trajectory reward normalization over %s query groups: min_trajectories=%s "
                        "max_trajectories=%s mean_trajectories=%.2f"
                    ) % (
                        len(trajectory_group_sizes),
                        min(trajectory_group_sizes),
                        max(trajectory_group_sizes),
                        sum(trajectory_group_sizes) / len(trajectory_group_sizes),
                    )
                    logger.info(summary_line)
                    logging_utils.append_progress_log(
                        self.args,
                        "[TrajectoryNorm] " + summary_line,
                    )

                normalized_rewards = [normalized_rewards_by_index[id(sample)] for sample in samples]
                return raw_rewards, normalized_rewards

            for group_index, group_samples in grouped_samples.items():
                group_rewards = [sample.get_reward_value(self.args) for sample in group_samples]
                rewards_tensor = torch.tensor(group_rewards, dtype=torch.float)
                rewards_tensor = rewards_tensor - rewards_tensor.mean()
                if self.args.advantage_estimator in ["grpo", "gspo"] and self.args.grpo_std_normalization:
                    if rewards_tensor.numel() <= 1:
                        logger.warning(
                            "Skipping std normalization for singleton reward group group_index=%s size=%s",
                            group_index,
                            rewards_tensor.numel(),
                        )
                    else:
                        rewards_tensor = rewards_tensor / (rewards_tensor.std() + 1e-6)
                for sample, normalized_reward in zip(group_samples, rewards_tensor.tolist(), strict=True):
                    normalized_rewards_by_index[id(sample)] = normalized_reward
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        train_data = {
            ...
            "raw_reward_group_sizes": [len(group) for group in group_by(samples, lambda s: s.group_index).values()],
            "truncated": [1 if sample.status == Sample.Status.TRUNCATED else 0 for sample in samples],
            "sample_indices": [sample.index for sample in samples],
        }

        if getattr(self.args, "turn_level_loss_weight_by_num_turns", False):
            is_turn_level = bool(samples) and any("turn_index" in (sample.metadata or {}) for sample in samples)
            if is_turn_level:
                trajectory_sizes_by_id: dict[Any, int] = {}
                for sample in samples:
                    metadata = sample.metadata or {}
                    trajectory_id = metadata.get("trajectory_id", sample.index)
                    num_turns = metadata.get("num_turns_in_trajectory")
                    if isinstance(num_turns, int) and num_turns > 0:
                        trajectory_sizes_by_id[trajectory_id] = num_turns

                if len(trajectory_sizes_by_id) < len({(sample.metadata or {}).get("trajectory_id", sample.index) for sample in samples}):
                    for trajectory_id, trajectory_samples in group_by(
                        samples, lambda s: (s.metadata or {}).get("trajectory_id", s.index)
                    ).items():
                        trajectory_sizes_by_id.setdefault(trajectory_id, len(trajectory_samples))

                train_data["sample_weights"] = [
                    1.0 / max(trajectory_sizes_by_id.get((sample.metadata or {}).get("trajectory_id", sample.index), 1), 1)
                    for sample in samples
                ]
...
def _log_eval_rollout_data(rollout_id, args, data, extra_metrics: dict[str, Any] | None = None):
    ...
    for key in data.keys():
        ...
        if (samples := data[key].get("samples")) is not None:
            ...
            non_aborted_rewards = [
                ...
                for sample in non_aborted_samples
                if sample.reward is not None and sample.get_reward_value(args) is not None
            ]
            num_success = sum(1 for reward in non_aborted_rewards if reward == 1)
            num_non_aborted = len(non_aborted_samples)
            log_dict[f"eval/{key}/num_success"] = num_success
            log_dict[f"eval/{key}/num_aborted"] = len(samples) - num_non_aborted
            log_dict[f"eval/{key}/num_non_aborted"] = num_non_aborted
            log_dict[f"eval/{key}/success_rate_excluding_aborted"] = (
                num_success / num_non_aborted if num_non_aborted else 0.0
            )
    ...
    for key in sorted(log_dict):
        ...
        if isinstance(value, int):
            eval_progress_fields.append(f"{key}={value}")
        elif isinstance(value, float):
            eval_progress_fields.append(f"{key}={value:.6g}")
    if eval_progress_fields:
        line = f"[EvalMetrics] rollout={rollout_id + 1} " + " ".join(eval_progress_fields)
        logging_utils.append_progress_log(args, line)
...
def _log_rollout_data(rollout_id, args, samples, rollout_extra_metrics, rollout_time):
    ...
    for key in [
        ...
        "rollout/raw_reward_std",
        "rollout/combined_reward_mean",
        "rollout/format_reward_mean",
        "rollout/judge_reward_mean",
    ]:
        value = log_dict.get(key)
        if value is not None:
            short_key = key.split("/")[-1]
            reward_progress_fields.append(f"{short_key}={value:.3f}")
    if reward_progress_fields:
        line = f"[RolloutReward] rollout={rollout_id + 1} " + " ".join(reward_progress_fields)
        logging_utils.append_progress_log(args, line)


def compute_metrics_from_samples(args, samples):
    response_lengths = [sample.effective_response_length for sample in samples]
    effective_sample_count = sum(1 for s in samples if not bool(s.remove_sample))
    ...
    log_dict["aborted_ratio"] = np.mean([int(s.status == Sample.Status.ABORTED) for s in samples]).item()
    log_dict["failed_ratio"] = np.mean([int(s.status == Sample.Status.FAILED) for s in samples]).item()
    log_dict["remove_sample_ratio"] = np.mean([int(bool(s.remove_sample)) for s in samples]).item()
    log_dict["effective_batch_size"] = effective_sample_count
    log_dict["effective_batch_ratio"] = (
        effective_sample_count / total_sample_count if total_sample_count > 0 else 0.0
    )

    terminate_reasons = [str((s.metadata or {}).get("terminate_reason", "")).lower() for s in samples]
    rollout_timeout_markers = (
    ...
    log_dict["timeout_sample_ratio"] = np.mean(
        [int(any(marker in reason for marker in rollout_timeout_markers)) for reason in terminate_reasons]
    ).item()
    log_dict["sandbox_error_ratio"] = np.mean(
        [int(any(marker in reason for marker in sandbox_error_markers)) for reason in terminate_reasons]
    ).item()
    log_dict["judge_timeout_ratio"] = np.mean(
        [int(bool((s.metadata or {}).get("judge_timeout", False))) for s in samples]
    ).item()
    log_dict["aborted_or_timeout_ratio"] = np.mean(
        [
            int(
                s.status == Sample.Status.ABORTED
                or any(marker in reason for marker in rollout_timeout_markers)
            )
            for s, reason in zip(samples, terminate_reasons, strict=True)
        ]
    ).item()
    return log_dict
...
def _compute_reward_metrics(args, all_samples: list[Sample]):
    ...
    raw_rewards = [sample.get_reward_value(args) for sample in all_samples if sample.reward is not None]
    if raw_rewards:
        raw_rewards_np = np.array(raw_rewards, dtype=np.float64)
        log_dict["raw_reward_mean"] = raw_rewards_np.mean().item()
        log_dict["raw_reward_std"] = raw_rewards_np.std().item()
        log_dict["raw_reward_max"] = raw_rewards_np.max().item()
        log_dict["raw_reward_min"] = raw_rewards_np.min().item()

    reward_meta = [
        (sample.metadata or {}).get("reward")
        for sample in all_samples
        if isinstance((sample.metadata or {}).get("reward"), dict)
    ]
    for reward_key in ["combined", "format", "judge"]:
        values = [meta.get(reward_key) for meta in reward_meta if meta.get(reward_key) is not None]
        if not values:
            continue
        values_np = np.array(values, dtype=np.float64)
        log_dict[f"{reward_key}_reward_mean"] = values_np.mean().item()
        log_dict[f"{reward_key}_reward_std"] = values_np.std().item()
...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    interesting_rewards = [str(round(r, 1)) if (r := g[0].get_reward_value(args)) is not None else "None" for g in interesting_sample_groups]
