...
def filter_long_prompt(origin_samples: list[Sample], tokenizer, processor, max_length: int | None) -> list[Sample]:
    ...
    def _get_tokenized_prompt_length(sample: Sample) -> int | None:
        ...
        if isinstance(prompt, list):
            ...
            try:
                ...
                logger.warning("Skipping max_length check for list prompt due to chat template error: %s", exc)
                return None
    ...
    if processor:
        ...
        filtered_samples = []
        if text_only:
            for sample in text_only:
                prompt_length = _get_tokenized_prompt_length(sample)
                if prompt_length is None or prompt_length <= max_length:
