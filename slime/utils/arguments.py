...
def _get_explicit_cli_options() -> set[str]:
    """Best-effort set of explicitly provided long CLI options."""
    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            parser.add_argument(
                ...
                type=int,
                default=16,
                help="Maximum interaction steps per browser trajectory/task.",
            )
            parser.add_argument(
                "--hf-checkpoint",
                type=str,
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Enable adaptive fresh-query sampling based on per-query success statistics, "
                    "anti-starvation recency bonus, and completed trajectory length bonus."
                ),
            )
            parser.add_argument(
            ...
            parser.add_argument(
                "--adaptive-query-weight-all-fail",
                type=float,
                default=0.2,
                help="Sampling weight for queries whose cumulative success rate is exactly 0.",
            )
            parser.add_argument(
                "--adaptive-query-weight-low",
                type=float,
                default=1.0,
                help="Sampling weight for queries with cumulative success rate in [0, 0.3).",
            )
            parser.add_argument(
                "--adaptive-query-weight-mid",
                type=float,
                default=1.4,
                help="Sampling weight for queries with cumulative success rate in [0.3, 0.7).",
            )
            parser.add_argument(
                "--adaptive-query-weight-high",
                type=float,
                default=0.7,
                help="Sampling weight for queries with cumulative success rate in [0.7, 1.0).",
            )
            parser.add_argument(
                "--adaptive-query-weight-all-success",
                type=float,
                default=0.2,
                help="Sampling weight for queries whose cumulative success rate is exactly 1.",
            )
            parser.add_argument(
                "--adaptive-query-stale-cap",
                type=int,
                default=50,
                help="Selection-step gap at which the anti-starvation recency bonus saturates.",
            )
            parser.add_argument(
                "--adaptive-query-stale-alpha",
                type=float,
                default=1.0,
                help="Strength of the anti-starvation recency bonus for stale queries.",
            )
            parser.add_argument(
                "--adaptive-query-length-alpha",
                type=float,
                default=0.3,
                help="Maximum strength of the completed-trajectory length bonus.",
            )
            parser.add_argument(
                "--adaptive-query-length-ramp-rollouts",
                type=int,
                default=30,
                help="Number of rollout steps used to ramp in the length bonus from 0 to full strength.",
            )
            parser.add_argument(
                "--adaptive-query-length-confidence-k",
                type=float,
                default=3.0,
                help="Confidence smoothing term for completed-length statistics. Larger values slow down the bonus.",
            )
            parser.add_argument(
                "--adaptive-query-length-cap",
                type=float,
                default=2.0,
                help="Upper bound for normalized completed trajectory length when computing the length bonus.",
            )
            # update weight
            parser.add_argument(
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=int,
                default=1,
                help="Number of PPO optimization epochs over the same rollout data.",
            )
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "For turn-level samples, multiply each sample's loss contribution by "
                    "1 / num_turns_in_trajectory so longer trajectories do not automatically "
                    "receive proportionally larger total weight."
                ),
            )
            parser.add_argument(
                "--kl-coef",
            ...
            parser.add_argument(
                ...
                type=str,
                default=None,
                help="Path to the custom TIS/RS function (e.g., your_package.off_policy:compute_importance_weights).",
            )
            parser.add_argument(
                "--custom-pg-loss-reducer-function-path",
                type=str,
                default=None,
                help="Path to a custom reducer function for pg_loss only. When set, pg_loss will use this custom reducer while other metrics (pg_clipfrac, ppo_kl, entropy_loss, etc.) still use the default sum_of_sample_mean. (e.g., your_package.loss_reducer:get_pg_loss_reducer).",
            )
        ...
        def add_wandb_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "If unset, wandb SDK default is used."
                ),
            )
            parser.add_argument(
        ...
        def add_debug_arguments(parser):
            ...
            parser.add_argument(
                "--log-megatron-image-token-debug",
                action="store_true",
                default=False,
                help=(
                    "Log Megatron-side multimodal/token alignment statistics during training, "
                    "including image-pad token counts and multimodal tensor presence."
                ),
            )
            parser.add_argument(
                "--log-megatron-image-token-debug-max-samples",
                type=int,
                default=3,
                help="How many local sample examples to print for Megatron image-token debug logging.",
            )
            parser.add_argument(
                "--dump-details",
                type=str,
        ...
        def add_rollout_processing_arguments(parser):
            parser.add_argument(
                "--rollout-task-type",
                type=str,
                default="math",
        ...
        parser = add_rollout_processing_arguments(parser)
        ...
        reset_arg(
            parser,
            "--context-num-screenshots",
            type=int,
            default=3,
            help="Browser turn-level context: number of latest screenshots to keep.",
        )
        reset_arg(
            parser,
            ...
            type=int,
            default=3,
            help="Browser judge: maximum number of screenshots attached to the judge prompt.",
        )
        reset_arg(
            parser,
            "--judge-timeout-secs",
            type=float,
            default=60.0,
            help="Browser judge: timeout in seconds for a single judge API call.",
        )
        reset_arg(
            parser,
            "--judge-api-model",
        ...
        reset_arg(
            ...
            help="Browser judge client mode. 'served' uses JUDGE_API_BASE or JUDGE_API_HOST/JUDGE_API_PORT.",
        )
        reset_arg(
            parser,
            ...
            help="Browser judge prompt variant.",
        )
        reset_arg(
            parser,
            ...
            help="Browser turn-level history compression mode for historical assistant reasoning.",
        )
        reset_arg(
            parser,
            "--browser-history-reasoning-max-turns",
            type=int,
            default=0,
            help=(
                "Browser turn-level rollout: keep reasoning according to --turn-history-reasoning-mode "
                "only for the most recent N historical assistant turns; older assistant turns hide "
                "thinking but keep non-thinking text. 0 disables this window and keeps current behavior."
            ),
        )
        reset_arg(
            parser,
            "--browser-tool-response-max-chars",
            type=int,
            default=512,
            help="Browser rollout: maximum number of characters kept for each tool response before truncation.",
        )
        reset_arg(
            parser,
            ...
            choices=[0, 1],
            help=(
                "Browser rollout: whether to include <tool_response>...</tool_response> "
                "feedback in the next user turn. 1 keeps current behavior; 0 sends only observation."
            ),
        )
        reset_arg(
            parser,
            ...
            help=(
                "Browser response format mode. "
                "'slime' keeps the current <thinking> + <action> + <tool_call> format; "
                "'browser_env' switches to a browser-env-style <think> + <tool_call> format."
            ),
        )
...
def slime_validate_args(args):
    ...
    if args.custom_config_path:
        explicit_cli_options = _get_explicit_cli_options()
        with open(args.custom_config_path) as f:
            data = yaml.safe_load(f) or {}
        for k, v in data.items():
            cli_option = f"--{k.replace('_', '-')}"
            if cli_option in explicit_cli_options:
                logger.info(
                    "Argument %s was explicitly set on the CLI; keeping %r and ignoring custom config value %r.",
                    k,
                    getattr(args, k, None),
                    v,
                )
