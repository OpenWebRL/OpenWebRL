...
def compute_pass_rate(
    ...
    if group_sizes is not None:
        ...
        if max(group_sizes) <= 1:
            return {}
        assert sum(group_sizes) == len(flat_rewards), f"{len(flat_rewards)=} does not match {sum(group_sizes)=}"
        split_rewards = []
        offset = 0
        for size in group_sizes:
            split_rewards.append(np.array(flat_rewards[offset : offset + size]))
            offset += size

        pass_rate_name_list = [2**i for i in range(int(math.log2(max(group_sizes))) + 1)]
        log_dict = {}
        for k in pass_rate_name_list:
            valid_groups = [group for group in split_rewards if len(group) >= k]
            if not valid_groups:
                continue
            num_correct = np.array([np.sum(group == 1) for group in valid_groups])
            num_samples = np.array([len(group) for group in valid_groups])
