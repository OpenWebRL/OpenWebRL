...
class BaseEnv(ABC):
    ...
    def get_obs(self, return_screenshot=True, return_a11ytree=False):
        assert (
            return_screenshot or return_a11ytree
        ), "At least one of return_screenshot and return_a11ytree should be True."
        ret = {}
        if return_screenshot:
            ret["screenshot"] = self.get_screenshot()
        if return_a11ytree:
            ret["a11ytree"] = self.get_a11ytree()
        return ret
